...
const packageName = process.env.UPDATER_PKG_NAME || "aicoyy";
const port = parseInt(process.env.UPDATER_PORT || "30129", 10);
...
const appPort = parseInt(process.env.UPDATER_APP_PORT || "30128", 10);

// Data directory (match mitm/paths.js logic)
function getDataDir() {
  if (process.env.DATA_DIR) return process.env.DATA_DIR;
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "aicoyy");
  }
  return path.join(os.homedir(), ".aicoyy");
}
